...
UdpL4Protocol::NotifyNewAggregate()
{
    ...
    Ptr<AHProtocol> ah = node->GetObject<AHProtocol>();
/*if (!ah) {
    // Ako `AHProtocol` još nije agregiran, kreiraj ga i dodaj.
    ah = CreateObject<AHProtocol>();
    node->AggregateObject(ah);
    NS_LOG_INFO("AHProtocol uspešno agregiran.");
} else {
    NS_LOG_INFO("AHProtocol već postoji, preskačem agregaciju.");
}*/
    ...
    if (!m_node)
    {
        ...
        {
            this->SetNode(node);
             NS_LOG_INFO("Usli smo u ovo za UDP");
   ...
   if(ah){ NS_LOG_INFO("Usli smo u ovo za AH"); this->SetDownTarget(MakeCallback(&AHProtocol::Send, ah));}else{
...
UdpL4Protocol::Receive(Ptr<Packet> packet, const Ipv4Header& header, Ptr<Ipv4Interface> interface)
{
    ...
    NS_LOG_DEBUG("Looking up dst " << header.GetDestination() << " port "
                                   << udpHeader.GetDestinationPort()<<" izvorisni port:" <<udpHeader.GetSourcePort());
    ...
    NS_LOG_DEBUG("Remaining payload size: " << packet->GetSize());
...
NS_LOG_INFO("Postoji tag"<<tag.GetFlag());
    if (tag.GetFlag()!=0) {
        NS_LOG_INFO("Imamo tag, započinjemo agregaciju AH protokola.");
        
        Ptr<Node> currentNode = this->GetObject<Node>();
        if (!currentNode) {
            NS_LOG_ERROR("Nije moguće dobiti trenutni čvor!");
            return;
        }

        // Proveri i agregiraj AH protokol za trenutni čvor
        Ptr<AHProtocol> ah = currentNode->GetObject<AHProtocol>();
        if (!ah) {
            ah = CreateObject<AHProtocol>();
            currentNode->AggregateObject(ah);
            NS_LOG_INFO("AHProtocol uspešno agregiran na čvoru " << currentNode->GetId());
        }

        // Iteriraj kroz sve čvorove u mreži i osiguraj da imaju AH protokol
        for (uint32_t i = 0; i < NodeList::GetNNodes(); ++i) {
            Ptr<Node> node = NodeList::GetNode(i);
            if (!node->GetObject<AHProtocol>()) {
                Ptr<AHProtocol> nodeAh = CreateObject<AHProtocol>();
                node->AggregateObject(nodeAh);
                NS_LOG_INFO("AHProtocol agregiran na čvoru " << node->GetId());
            }
        }
    }
}
    ...
    udpHeader.SetDestinationPort(dport);
    udpHeader.SetSourcePort(sport);
NS_LOG_DEBUG("Packet size before adding UDP header: " << packet->GetSize());
    packet->AddHeader(udpHeader);
NS_LOG_DEBUG("Packet size after adding UDP header: " << packet->GetSize());
